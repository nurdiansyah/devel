...
import Api from "ava/lib/api.js";
import { chalk } from "ava/lib/chalk.js";
import validateEnvironmentVariables from "ava/lib/environment-variables.js";
import normalizeExtensions from "ava/lib/extensions.js";
import { normalizeGlobs, normalizePattern } from "ava/lib/globs.js";
import { controlFlow } from "ava/lib/ipc-flow-control.cjs";
import isCi from "ava/lib/is-ci.js";
import { splitPatternAndLineNumbers } from "ava/lib/line-numbers.js";
import { loadConfig } from "ava/lib/load-config.js";
import normalizeModuleTypes from "ava/lib/module-types.js";
import normalizeNodeArguments from "ava/lib/node-arguments.js";
import pkg from "ava/lib/pkg.cjs";
import providerManager from "ava/lib/provider-manager.js";
import DefaultReporter from "ava/lib/reporters/default.js";
import TapReporter from "ava/lib/reporters/tap.js";
import Watcher from "ava/lib/watcher.js";
...
export default async function loadCli() {
  ...
  try {
    ...
    if (
      conf.configFile &&
      path.basename(conf.configFile) !== path.relative(conf.projectDir, conf.configFile)
    ) {
      console.log(
        chalk.magenta(`  ${figures.warning} Using configuration from ${conf.configFile}`)
      );
    }
  } catch (error) {
    confError = error;
  ...
  if (combined.color !== false) {
    const {
      supportsColor: { level }
    } = await import("chalk"); // eslint-disable-line node/no-unsupported-features/es-syntax, unicorn/import-style
  ...
  if (
    Reflect.has(combined, "concurrency") &&
  ...
  if (Reflect.has(conf, "helpers")) {
    exit(
      "AVA no longer compiles helpers. Add exclusion patterns to the ’files’ configuration and specify ’compileAsTests’ in the Babel options instead."
    );
  }
  ...
  try {
    globs = normalizeGlobs({
      files: conf.files,
      ignoredByWatcher: conf.ignoredByWatcher,
      extensions,
      providers
    });
  } catch (error) {
  ...
  const reporter =
    combined.tap && !combined.watch && debug === null
      ? new TapReporter({
  ...
  api.on("run", (plan) => {
    ...
    if (
      process.env.AVA_EMIT_RUN_STATUS_OVER_IPC ===
      "I'll find a payphone baby / Take some time to talk to you"
    ) {
